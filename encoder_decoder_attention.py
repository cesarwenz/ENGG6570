...
def define_generator(in_shape, vector_shape, latent_dim, n_classes=7):
    ...
    d = LSTM(200, activation='relu', return_sequences=True, recurrent_initializer='glorot_uniform')(b)
    m = Dense(200, use_bias=False)(d)
    t = LSTM(200, activation='relu', return_sequences=True, recurrent_initializer='glorot_uniform')(d)
    t = Dense(200, use_bias=False)(t)
    attention = AdditiveAttention()([t, d, m])
    
    p = Dense(200, activation='tanh', use_bias=False)(attention)
    # output
    out_layer = Dense(3)(p)
